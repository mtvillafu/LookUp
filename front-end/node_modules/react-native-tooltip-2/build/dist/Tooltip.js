...
import { computeBottomGeometry, computeCenterGeometry, computeLeftGeometry, computeRightGeometry, computeTopGeometry, makeChildlessRect, Point, Rect, Size, swapSizeDimensions, } from "./geometry";
...
class Tooltip extends Component {
    ...
    constructor(props) {
        super(props);
        const { isVisible, useInteractionManager = Tooltip.defaultProps.useInteractionManager, } = props;
        ...
        this.state = {
            ...
            placement: React.Children.count(props.children) === 0
    ...
    static getDerivedStateFromProps(nextProps, prevState) {
        ...
        const nextPlacement = React.Children.count(nextProps.children) === 0
    ...
    doChildlessPlacement = () => {
        this.onChildMeasurementComplete(makeChildlessRect({
    ...
    computeGeometry = () => {
        const { arrowSize = Tooltip.defaultProps.arrowSize, childContentSpacing } = this.props;
        const { childRect, contentSize, displayInsets, placement, windowDims } = this.state;
        const options = {
            displayInsets,
            childRect,
            windowDims,
            arrowSize: placement === Placement.TOP || placement === Placement.BOTTOM
                ? arrowSize
    ...
    renderChildInTooltip = () => {
        ...
        return (<TooltipChildrenContext.Provider value={{ tooltipDuplicate: true }}>
        <View onTouchEnd={onTouchEnd} pointerEvents={this.props.allowChildInteraction ? "box-none" : "none"} style={[
                {
                    position: "absolute",
                    height,
                    width,
    ...
    renderContentForTooltip = () => {
        ...
        return (<TouchableWithoutFeedback onPress={onPressBackground} accessible={this.props.accessible}>
    ...
    render() {
        ...
        {useReactNativeModal ? (<ModalComponent transparent visible={showTooltip} onRequestClose={this.props.onClose} supportedOrientations={this.props.supportedOrientations}>
            {this.renderContentForTooltip()}
          </ModalComponent>) : null}

        {/* This renders the child element in place in the parent's layout */}
        {hasChildren ? (<View ref={this.childWrapper} onLayout={this.measureChildRect} style={this.props.parentWrapperStyle}>
            {children}
          </View>) : null}
